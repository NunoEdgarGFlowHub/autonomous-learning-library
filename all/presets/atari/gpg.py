...
from .models import nature_cnn, nature_policy_head
...
def gpg(
    ...
    def _gpg_atari(env, writer=DummyWriter()):
        feature_model = nature_cnn().to(device)
        value_model = nn.Linear0(513, 1).to(device)
        ...
        return DeepmindAtariBody(
            GPG(features, v, policy, gamma=discount_factor, min_batch_size=min_batch_size, writer=writer),
            env
        )
    return _gpg_atari
