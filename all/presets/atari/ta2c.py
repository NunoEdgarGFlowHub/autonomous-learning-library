...
def ta2c(
    ...
    def _ta2c(envs, writer=DummyWriter()):
        ...
        long_model = value_net().to(device)
        ...
        long_optimizer = RMSprop(long_model.parameters(), alpha=alpha, lr=lr, eps=eps)
        ...
        v_plus = ValueNetwork(
            long_model,
            long_optimizer,
            loss_scaling=long_loss_scaling,
            clip_grad=clip_grad,
            writer=writer
        )
